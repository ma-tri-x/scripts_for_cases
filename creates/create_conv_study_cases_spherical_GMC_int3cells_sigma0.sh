...
for i in $study_cases;do
    ...
    sed -i "s/\"sigma\": .*/\"sigma\": 0.0/g" conf_dict.json
    ## decompose is on from 0.2mum on downwards
    ...
    #
    sed -i "s/\"MassCorr\": \"local\",/\"MassCorr\": \"global\",/g" conf_dict.json
    ...
    sed -i "s/outputInterval  1;/outputInterval  50;/g" system/controlDict.template
    rm Allrun.template.*
    echo "creating $i"
    bash rerun.sh -par
    Rn=$(cat Rn_export)
...
for i in $study_cases;do
    echo $i
    cat $i/conf_dict.json | grep "\"sigma\":"
done
