...
deltas=(0 0 0 -0.3e-6 -0.3e-6 -1.2e-6)
...
do
    ...
    do
        l=0
        let 'l=k-1'
        ind=$(echo "$l % ($case_count )" | bc)
        the_time=$(m4 <<< "esyscmd(perl -e 'printf( $time_in + (${deltas[$ind]})  )')")
        echo "$j: $ind:   $time_in + ${deltas[$ind]} = $the_time"
        data=$(get_contour_path $j $the_time)
...
convert contour.png -background white -flatten contour_SolidBoundary_different_times.jpg
rm contour*.png
