...
fit [0.7:*] f(x) "calc_times7.dat" u 1:(($2)/60.*($4)/101e-6) via a,b

set logscale y

p \
f(x) t sprintf("%.1f /(res.)^{%.1f} + %.1f",a,b,c),\
     "calc_times7.dat"  u 1:(($2)/60.*($4)/101e-6) w p ps 2 lw 3 t "times Ryzen",\
     "calc_times7.dat"  u 1:(($3)/1e3) w p ps 2 pt 2 lw 3 lc 8 axis x1y2 t "amount of cells"
