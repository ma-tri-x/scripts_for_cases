...
#a = np.genfromtxt("parameter_scan_exclude_gamma2.5.dat",usecols=(5,6,8))
...
# use RBF
rbf = RBFInterpolator(ag, Tc, epsilon=2)
z_i = rbf(xy_i)
...
lims = dict(cmap='terrain', vmin=1.0, vmax=1.2) #vmin=np.min(Tc), vmax=np.max(Tc)
...
CS = ax.contour(X_edges[:-1,:-1], Y_edges[:-1,:-1], z_i.reshape(100, 100),
                levels=[1.02,1.04,1.06,1.08,1.1,1.14,1.17,1.18],
                linewidths=4,
                colors=('black')
                )
ax.clabel(CS, inline=True, fontsize=19, colors='k')

ax.scatter(alphar, gamma, 100, Tc, edgecolor='black', lw=2.0, **lims)

ax.set(
    title='$t_{{V_{{min}}}}$ [$2 T_{{Rc}}$]',
    xlim=(0.27, 8.2),
    ylim=(0.27, 2.37)
)
...
#os.system("convert Tc.png -trim Tc.png")
