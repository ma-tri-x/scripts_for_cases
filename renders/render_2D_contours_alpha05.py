...
def render_timestep():
    ...
    state_file = os.path.join(fpath, "states/contour_export_alpha05.pvsm")
    state_file_backup = os.path.join(fpath, "states/contour_export_alpha05.pvsm.backup")
    case_name = fpath.split("/")[-1]
    keyword = "PlotOnIntersectionCurves1"
    
    if not os.path.isfile(state_file_backup):
        print("error: states/contour_export_alpha05.pvsm.backup file does not exist")
        exit(1)
